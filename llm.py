    ...
    while wait < self.request_timeout:
      try:
        if self.model_name == 'fireworks-llama-v3-70b-instruct':
          ...
          logger.info(f'Fireworks API Call. Finish Model: {self.model_name}. User Prompt: {user_prompt}, Output: {response}')
          ...
          logger.info(f'Fireworks API Call. Finish Model: {self.model_name}. User Prompt: {user_prompt}, Output: {formatted_response}')



        else:
          logger.critical('The specified model does not exist.')
        ...
        return response
          ...
          print(e)
          logger.error(f'class oaiLLM/predict(), Error, {e}')
          logger.info(f'Waiting for {str(wait)} seconds...')
          time.sleep(wait)
          wait *= 2
          continue
