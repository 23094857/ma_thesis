...
class GA:
    def __init__(self, llm_evaluation, llm_assistent, dataset, population: Optional[List[str]]=None, population_size=16,
                 ...
                 baseline_accuracy=0.41, HPS_directory=None, reset_evals_every_n_llm_calls=(1e10), trial=None, pruning=False):
        ...
        assert self.max_llm_calls > self.evaluations_per_generation * self.population_size, "max_llm_calls should be greater than evaluations_per_generation * population_size."
        self.min_avrg_accuracy_increase_per_10_generations = min_avrg_accuracy_increase_per_10_generations
        self.HPS_directory = HPS_directory
        self.ga_logger, self.log_directory = self.setup_logger("GA_logger", "ga.log", HPS_folder=self.HPS_directory)
        self.llm_logger, _ = self.setup_logger("LLM_logger", "llm.log", HPS_folder=self.HPS_directory)
    ...
    def setup_logger(self, name, log_file, level=logging.INFO, log_directory=None, HPS_folder=None):
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        if log_directory is not None:
            raise ValueError("log_directory should be None; use HPS_folder instead.")

        # Set log_directory based on HPS_folder or default to ./logs/
        if HPS_folder is None:
            log_directory = f"./logs/{timestamp}"
        else:
            log_directory = f"{HPS_folder}/{timestamp}"
    ...
    def evaluate_problem(self, prompt):
        index_random_problem = random.randint(0, len(self.dataset)-1)
        problem = self.dataset[index_random_problem]
        question = problem['question']
        solution = problem['answer']
        answer = self.llm_evaluation.predict(problem['question'] + " " + prompt, self.llm_logger, max_tokens=1000)
        correct = False
        if answer is None:
            self.llm_logger.error(f"LLM call failed for prompt: {prompt}")
        if question[-2] + '=' + solution in "".join(answer.split()):
            correct = True

        if correct == True: self.increase_pos_eval_of_current_gen_by_one(prompt)
        if correct == False: self.increase_neg_eval_of_current_gen_by_one(prompt)
    ...
    def evaluate_current_population(self, n_evals=None):
            ...
            n_evals = n_evals if n_evals is not None else self.evaluations_per_generation
    ...
    def log_parameters_GA(self):
        ...
        self.ga_logger.info(f'evaluations_per_generation: {self.evaluations_per_generation}')
    ...
    def run_GA(self):
        ...
        try:
            ...
            if self.evaluations_per_generation < self.max_evaluations_per_prompt:
                ...
                rounds = self.max_evaluations_per_prompt // self.evaluations_per_generation
